...
MEASEQ = [NET_CAPs, PHY_MEM, IO_CAPs, CPU_CAPs];
...
class Utiltools:
	...
	def generateRadomTestData(cls, vmn):
		...
		for i in range(len(MEASEQ)):
			pm.write(str(MEASEQ[i]+"\n")	
		
		for i in range(int(vmn)):
			vm = open("../data/vm"+str(i)+".txt", "w");
			vm.write("net mem io cpu\n");
			vm.write(str(MEASEQ[0]*random.random())+" "+str(MEASEQ[1]*3*random.random())+" "+str(MEASEQ[2]*random.random())+" "+str(MEASEQ[3]*3*random.random()));
	...
	def readForOneVirtual(cls, fileName, no):
		...
		if(dsrc == "random"):
			for line in f.readlines():
				sample = dict();
				if(count>0):
					sample["NET_CAPs"] = float(line.split()[0]);
					sample["PHY_MEM"] = float(line.split()[1]);	
					sample["IO_CAPs"] = float(line.split()[2]);	
					sample["CPU_CAPs"] = float(line.split()[3]);	
			...
			for line in f.readlines():
				...
				data = line.split("\t");
				sample["NET_CAPs"] = 0;
				sample["PHY_MEM"] = data[1];	
				sample["IO_CAPs"] = data[2];	
				sample["CPU_CAPs"] = data[3];	
				res_request.append(sample);	
...
class GeneticAlgorithm:
	...
	def assigngroup(self, assignment, k):
		...
		for i in range(len(self.pdgroup)):
			if(self.pdassignmentvect[self.pdgroup[i]] < pdselectnum):
				self.cur_groups[assignment[self.pdgroup[i]]].append(self.cur_pops[i]);
				self.pdassignmentvect[self.pdgroup[i]] += 1; 
	...
	def ifPdVectSame(self, vect1, vect2):
        ...
        def ifPdVectContained(self, lst, vect, j):
		for i in xrange(len(lst)):
			if(self.ifPdVectSame(lst[i], vect)):
				self.pdgroup[j] = i
				return True;
		return False;
	...
	def k_means_classify(self, k):
		...
		lst = [];
		self.initialgroup(k)
		self.pdgroup = [0]*len(self.cur_pops);
		...
		for i in range(len(self.cur_pops)):
			...
			if(self.ifPdVectContained(lst, sample, i) is not True):
				lst.append(sample);
				self.pdgroup[i] = pdlen;
				pdlen += 1;
		
		self.pdassignmentvect = [0]*len(lst)
...
class classifier:
	...
	def logvect(self, vect, info):
		f = open("../log/"+info+".log", "a");
	...
	def initializecvec(self, vmlist):
		for i in range(len(vmlist)):
			...
			temp = [];
			x0 = {};
			x0[1] = float(vmlist[i].res_request[0]["NET_CAPs"]);
			x0[2] = float(vmlist[i].res_request[0]["PHY_MEM"]);
			x0[3] = float(vmlist[i].res_request[0]["IO_CAPs"]);
			x0[4] = float(vmlist[i].res_request[0]["CPU_CAPs"]);
			x.append(x0)
			print("-------------"+str(x)+"------------\n");
			...
			temp.append(cdict["net"])
			temp.append(cdict["mem"])
			temp.append(cdict["io"])
			temp.append(cdict["cpu"])
			self.logvect(x[0].values(), "nonclassvect");
			self.logvect(temp, "classvect");
...
if __name__ == "__main__":
	...
	parser.add_argument("--pdselectnum", help="the number of selected d vect whose pd is the same one");
