...
class Utiltools:
	...
	def generateRadomTestData(cls, vmn): 
		...
		for i in range(len(MEASEQ)):
			pm.write(str(MEASEQ[i]*2)+"\n")
		pm.close();

		for i in range(int(vmn)):
			vm = open("../data/vm"+str(i)+".txt", "w");
			vm.write("net mem io cpu\n");
			vm.write(str(MEASEQ[0]*random.random())+" "+str(MEASEQ[1]*random.random())+" "+str(MEASEQ[2]*random.random())+" "+str(MEASEQ[3]*random.random()));
...
class classifier:
	...
	def initializecvec(self, vmlist):
		for i in range(len(vmlist)):
			...
			x0 = {};
			y = [10];
			x0[1] = float(vmlist[i].raw_res[0]);
			x0[2] = float(vmlist[i].raw_res[1]);
			x0[3] = float(vmlist[i].raw_res[2]);
			x0[4] = float(vmlist[i].raw_res[3]);
			print("*****"+str(vmlist[i].raw_res[0])+" "+str(vmlist[i].raw_res[1])+" "+str(vmlist[i].raw_res[2])+" "+str(vmlist[i].raw_res[3])+"\n")
			x.append(x0)
			...
			self.logvect(vmlist[i].class_vect, "classvect");
